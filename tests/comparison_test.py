...
def main():
    ...
    for row in docs:
        ...
        for c_i, chunk in enumerate(chunks):
            ...
            prepared_data.append(
                {"doc_id": idx, "chunk_idx": c_i, "text": chunk, "vector": vec}
            )
    ...
    try:
        ...
        try:
            w_client.delete_collection(WADDLE_COL)
        except:
            pass
        w_collection = w_client.create_collection(WADDLE_COL, 384)
    except Exception as e:
    ...
    c_collection = c_client.create_collection(
        name=CHROMA_COL, metadata={"hnsw:space": "cosine"}
    )  # Use cosine to match
    ...
    for item in prepared_data:
        ...
        if len(w_items) >= w_batch_size:
            try:
                w_collection.batch_append_blocks(w_items)
            except Exception as e:
    ...
    print(
        f"  WaddleDB Ingestion: {w_ingest_time:.4f}s ({len(prepared_data)/w_ingest_time:.1f} chunks/s)"
    )
    ...
    print(
        f"  ChromaDB Ingestion: {c_ingest_time:.4f}s ({len(prepared_data)/c_ingest_time:.1f} chunks/s)"
    )
    ...
    for name, questions in scenarios:
        ...
        for q_item in questions:
            ...
            # Waddle Search
            sw = time.time()
            w_res = w_collection.search(q_vec, top_k=5)
    ...
    print("\n--- Comparative Results ---")
    print(
        f"{'Scenario':<15} | {'Waddle Lat':<10} | {'Chroma Lat':<10} | {'Waddle Rec':<10} | {'Chroma Rec':<10}"
    )
    print("-" * 65)
    for r in results:
        print(
            f"{r['Scenario']:<15} | {r['W_Lat']:.2f} ms    | {r['C_Lat']:.2f} ms    | {r['W_Rec']:.1%}     | {r['C_Rec']:.1%}"
        )
