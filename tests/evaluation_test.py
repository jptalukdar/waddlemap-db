...
try:
    ...
    print("Error: Could not import WaddleClient. Make sure you are running this from the tests directory or have the python client generated.")
    sys.exit(1)
...
def evaluate_scenario(client, model, name, questions, k=5):
    ...
    for i, item in enumerate(questions):
        ...
        try:
            results = client.search(COLLECTION_NAME, q_vec, top_k=k)
...
def main():
    ...
    # 1. Single Passage
    res = evaluate_scenario(client, model, "Single Passage", qa_single)
    results.append(res)
    
    # 2. Multi Passage
    res = evaluate_scenario(client, model, "Multi Passage", qa_multi)
    results.append(res)
    ...
    res = evaluate_scenario(client, model, "No Answer", qa_no_answer)
    results.append(res)
    
    print("\n--- Final Summary ---")
    print(f"{'Scenario':<15} | {'Avg Lat (ms)':<12} | {'P95 Lat (ms)':<12} | {'Recall@5':<10} | {'Avg Dist':<10}")
    print("-" * 70)
    for r in results:
        print(f"{r['Scenario']:<15} | {r['AvgLatencyMs']:<12.2f} | {r['P95LatencyMs']:<12.2f} | {r['RecallAt5']:<10.2%} | {r['AvgDist']:<10.4f}")
